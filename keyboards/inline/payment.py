...
def ikb_athlete_payment_amount(trainer: Trainer, callback_data: AthletePayment, active: bool):
    athletes_list = sorted(trainer.athletes_active if active else trainer.athletes_inactive, key=lambda x: x.first_name)
    current_user = athletes_list[callback_data.athlete_id]
    keyboard = InlineKeyboardBuilder()
    add_navigation_button(keyboard, '<<<', 'pay_request' if active else 'pay_request_inactive',
                          callback_data.athlete_id, (callback_data.pay_amount - 1) % 15)
    keyboard.button(text=f'Оплатить: {callback_data.pay_amount}',
                    callback_data=AthletePayment(button='confirm' if active else 'confirm_inactive',
                                                 athlete_id=callback_data.athlete_id,
                                                 pay_amount=callback_data.pay_amount))
    add_navigation_button(keyboard, '>>>', 'pay_request' if active else 'pay_request_inactive',
                          callback_data.athlete_id, (callback_data.pay_amount + 1) % 15)
...
#                                                        athlete_id=callback_data.athlete_id,
#                                                        current_month=-1))
#         keyboard.button(text='Оплата', callback_data=AthletePayment(button='pay_request',
#                                                                     athlete_id=callback_data.athlete_id,
#                                                                     pay_amount=12))
...
def add_navigation_button(ikb: InlineKeyboardBuilder,
                          ...
                          athlete_id: int = 0, pay_amount: int = 0):
    ikb.button(text=button_text, callback_data=AthletePayment(button=button_cb,
                                                              athlete_id=athlete_id, pay_amount=pay_amount))
#
#
