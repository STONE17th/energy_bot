...
from keyboards import ikb_athlete_payment_amount, ikb_athletes_profile_navigation
from keyboards.inline.callbackdata import TrainerMainMenu, TrainerMenu, AthletesMenuNavigation, AthletePayment
from classes import *
...
@payment_router.callback_query(AthletePayment.filter(F.button.in_({'pay_request', 'pay_request_inactive'})))
async def payment_request(callback: CallbackQuery, callback_data: AthletePayment, user: Trainer, bot: Bot):
    switch = True if callback_data.button == 'pay_request' else False
    athletes_list = sorted(user.athletes_active if switch else user.athletes_inactive, key=lambda x: x.first_name)
    current_athlete = athletes_list[callback_data.athlete_id]
    message_text = f'{current_athlete.first_name} {current_athlete.last_name} '
    message_text += f'оплатил {callback_data.pay_amount} тренировок'
    message_media = InputMediaPhoto(media=current_athlete.photo, caption=message_text)
    await bot.edit_message_media(media=message_media, chat_id=callback.from_user.id,
                                 message_id=callback.message.message_id,
                                 reply_markup=ikb_athlete_payment_amount(user, callback_data, switch))


@payment_router.callback_query(AthletePayment.filter(F.button.in_({'confirm', 'confirm_inactive'})))
async def payment_confirm(callback: CallbackQuery, callback_data: AthletePayment, user: Trainer, bot: Bot):
    switch = True if callback_data.button == 'confirm' else False
    athletes_list = sorted(user.athletes_active if switch else user.athletes_inactive, key=lambda x: x.first_name)
    athletes_list[callback_data.athlete_id].paid(callback_data.pay_amount)
    await callback.answer(f'Оплата за {callback_data.pay_amount} тренировок принята', show_alert=True)
    if athletes_list:
        current_athlete = athletes_list[callback_data.athlete_id]
        message_text = f'[{callback_data.athlete_id + 1}/{len(athletes_list)}]\n'
        message_text += f'{current_athlete.first_name} {current_athlete.last_name}\n'
    ...
    await bot.edit_message_media(media=message_media, chat_id=callback.from_user.id,
                                 message_id=callback.message.message_id,
                                 reply_markup=ikb_athletes_profile_navigation(user, callback_data, switch))
    # print(callback_data.athlete_id, callback_data.pay_amount)
